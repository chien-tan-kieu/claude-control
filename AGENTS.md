# Throughline — Project Guide

## Package Manager

**Bun only.** This project uses bun as both runtime and package manager.

- Install: `bun install` (lockfile: `bun.lock`)
- Add a root dev dep: `bun add -d <package>` (run from repo root)
- Add a workspace dep: `bun add <package> --cwd packages/<name>`
- `pnpm` and `npm` are NOT used — do not run `pnpm install` or `npm install`

## Workspace Structure

```
throughline/          ← repo root (bun workspaces host)
  packages/
    server/              ← @throughline/server  — Bun HTTP daemon (bun:test)
    web/                 ← @throughline/web     — React dashboard (vitest)
    shared/              ← @throughline/shared  — Shared TypeScript types
  plugin/                ← Command Code plugin (skills, commands, hooks)
  scripts/               ← Release utility scripts (sync-version.mjs, extract-changelog.mjs)
  .github/workflows/     ← CI (ci.yml) and release (release.yml) pipelines
```

Workspace membership is declared in root `package.json → "workspaces": ["packages/*"]`. There is no `pnpm-workspace.yaml`.

## Test Commands

| Scope | Command |
|-------|---------|
| Server only | `cd packages/server && bun test` |
| Web only | `cd packages/web && bun run test` |
| All packages | `bun --filter '*' test` (from repo root) |

Server uses **bun:test** (`bun test`). Web uses **vitest** (`bun run test` — vitest is invoked via the `test` script). Do not run `bun test` in the web directory; it will use the wrong runner.

## Root Scripts

```bash
bun run dev      # Start server in watch mode
bun run build    # Build web dashboard
bun run test     # Run all package tests
bun run lint     # Biome linter
```

## Release Workflow

Version is authoritative in root `package.json`. Releases are cut manually from GitHub Actions — there is no local release command, and pushing to `main` no longer triggers a release (CI tests still run on every push).

To cut a release: **Actions tab → Release workflow → Run workflow**, choose `version_bump` (`patch` / `minor` / `major`), run on `main`. Or via CLI: `gh workflow run release.yml -f version_bump=patch`.

The workflow (`.github/workflows/release.yml`):
1. Runs the server and web test suites
2. Predicts the next version from the bump kind (dry run — no writes)
3. Fails fast if that version's tag or GitHub Release already exists
4. Bumps root `package.json`, propagates via `scripts/sync-version.mjs` to:
   - `packages/server/package.json`
   - `packages/web/package.json`
   - `packages/shared/package.json`
   - `plugin/plugin.json`
   - `.claude-plugin/plugin.json`
   - `.claude-plugin/marketplace.json`
   - `packages/server/src/index.ts` (`const VERSION`)
   - `README.md` (shields.io version badge)
   prepends a `CHANGELOG.md` entry, commits `chore: release vX.Y.Z`, and pushes to `main`
5. Builds the server bundle and web dashboard, assembles the `dist` branch, tags it, and pushes both
6. Creates the GitHub Release from the `dist` tag

If a run fails *after* step 4 has pushed the version-bump commit but before the release is published, do not immediately re-run the workflow — it will bump past the stuck version.

## Server Entry Point

`packages/server/src/index.ts` — exports `startDaemon()`. The daemon:
- Binds to port range 47821–47830
- Writes `.throughline/runtime.json` (port, token, pid, version)
- Exposes `GET /api/healthz` and `GET /api/status` without auth
- All other `/api/*` routes require `Authorization: Bearer <token>`

## graphify

This project has a graphify knowledge graph at graphify-out/. Head-to-head testing against grep on this repo (2026-07-27, 530 nodes / 869 edges) showed graphify is NOT a universal win — it excels at some query shapes and actively misleads on others. Pick the tool by query shape, not by default preference:

- **USE `graphify path "<A>" "<B>"`** when you have two known symbol/node names and want the connecting chain (e.g. "how does startDaemon reach checkAuth"). This was the clearest win in testing: a precise 3-hop answer at ~40 tokens vs. ~900 tokens of grep output that still required manual sifting to find the same chain. Use exact node labels — check `graphify-out/GRAPH_REPORT.md`'s god-node list or `graph.json` if unsure; an off-label guess returns "No path found" rather than a fuzzy match.
- **USE `graphify explain "<concept>"`** when you have one known exact node name and want its callers/dependents. Good signal, but neighbor labels can be ambiguous — e.g. multiple unrelated files both labeled `index.ts` with no path shown. When the exact file matters, confirm with `rg -n "<symbol>"` before acting on it.
- **USE `graphify query "<question>"` only with a verification step.** For broad natural-language questions ("how does X work") it can silently mis-seed on an off-topic community and return a confidently-formatted but wrong-domain answer — no error, no low-confidence signal. Reproduced in testing: "how does authentication work" returned nodes about the release workflow, not the auth code, on repeated runs. Before trusting a `query` result, check that the returned seed nodes actually match the question's topic; if they don't, discard it and fall back to `rg -i "<keyword>"` + reading the hit file. In the tested case, that grep fallback was both cheaper (~750 vs ~1000 tokens) and correct.
- **SKIP graphify, use grep/rg directly** for: literal strings, error messages, or exact text matches (not a concept or relationship); when the target file/directory is already known; broad "how does X work" conceptual questions (start with `rg -i "<keyword>"` to find the entry point, then read it — faster and more reliable than `graphify query` per above); or when `graphify-out/graph.json` does not exist / `graphify-out/.graphify_root` doesn't match `pwd` (stale — treat as absent, do a full rebuild, don't fall back to grep silently without noting the graph is unavailable).

Read `graphify-out/GRAPH_REPORT.md` for god nodes and community structure before any broad architecture question, but treat it as a starting point, not a substitute for verifying a `query` result against the actual question. If `graphify-out/wiki/index.md` exists, navigate it instead of reading raw files.

Maintenance:
- After modifying code files in this session, run `graphify update .` to keep the graph current (AST-only, no API cost, code files only).
- Doc/spec/plan changes are NOT covered by `graphify update .` — that's AST-only. Run the full `/graphify --update` pass (dispatches LLM subagents) after doc/spec/plan edits, or those nodes silently go stale.
- If `graphify-out/.graphify_root` doesn't match `pwd`, the graph was built under a different path (e.g. a repo rename) — treat it as stale and do a full rebuild rather than `--update`.
