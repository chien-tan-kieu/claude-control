...
class StoryService {
  ...
  upsertRow(id, filePath, title, status, size, linkedSpec, linkedPlan) {
    const ts = Date.now();
    const existing = this.db.query("SELECT seq FROM stories WHERE id = ?").get(id);
    ...
    this.db.run(`INSERT OR REPLACE INTO stories (id, file_path, title, size, status, linked_spec_path, linked_plan_path, created_at, updated_at, seq)
       VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE((SELECT created_at FROM stories WHERE id = ?), ?), ?, ?)`, [id, filePath, title, size, status, linkedSpec, linkedPlan, id, ts, ts, seq]);
  }
}
